...
const PatientDashboard = () => {
    ...
    const fetchUserData = async () => {
      try {
        const response = await axios.get(
          "http://localhost:8000/api/v1/dataRequest/getUserData",
          {
            headers: {
  ...
  useEffect(() => {
    const fetchHealthData = async () => {
      try {
        const response = await axios.get(
          "http://localhost:8000/api/v1/healthData/getHealthData",
          {
            headers: {
              Authorization: `Bearer ${token}`,
            },
  ...
  const handleFileUpload = async (event) => {
    ...
    try {
      await axios.post(
        "http://localhost:8000/api/v1/healthData/uploadHealthData",
        formData,
        {
          headers: {
    ...
    <div className="max-w-7xl mx-auto space-y-6">
      ...
      <div className="flex flex-col md:flex-row gap-2">
        ...
        <Card className="bg-gradient-to-br from-green-600 to-green-800 text-white">
          <CardContent className="p-6">
            <div className="flex flex-col md:flex-row items-center justify-between">
              <div className="space-y-2 mb-4 md:mb-0 flex items-start">
                <div>
                  <h3 className="text-xl font-semibold">
                    Current Studies & Clinical Trials
                  </h3>
                  <p className="text-blue-100">
                    Discover clinical trials seeking participants that match
                    your health profile
                  </p>
                </div>
              </div>
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          ...
          <p className="text-gray-500 capitalize">Welcome, {userData.name}</p>
        </div>
        ...
        <Card>
          ...
          <CardContent>
            <div className="flex items-center space-x-2">
              ...
              >
                {userData.kycStatus.charAt(0).toUpperCase() +
                  userData.kycStatus.slice(1)}
              </div>
            </div>
            <p className="text-xs text-gray-500 mt-1">
              {getKycStatusText(userData.kycStatus)}
            </p>
          </CardContent>
        </Card>
                          ...
                          case "doc":
                            return (
                              <div className="text-center py-8">
                                <p>
                                  This is a .docx file. You can download it:
                                </p>
                                <a
                                  href={`${filePath}`}
                                  download
      ...
      {/* KYC Details/Status Card - Shows more detailed KYC information */}
          ...
          <CardHeader>
            ...
            <CardDescription>
              {userData.kycStatus === "pending"
                ? "Your identity verification is in progress. This process usually takes 1-2 business days."
          ...
          <CardContent>
            <div
              className={`p-4 rounded-lg ${
                userData.kycStatus === "pending" ? "bg-yellow-50" : "bg-red-50"
              }`}
            >
              <div className="flex items-start space-x-4">
                ...
                <div>
                  ...
                  <p className="text-sm text-gray-600 mt-1">
                    {userData.kycStatus === "pending"
                      ? "We are currently reviewing your submitted documents. Once verified, you will have full access to all platform features."
                      : "Your documents were rejected. Common reasons include unclear images, mismatched information, or expired documents. Please resubmit with clear, valid documents."}
                  </p>
